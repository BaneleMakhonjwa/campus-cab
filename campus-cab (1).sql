...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 11, 2026 at 10:18 PM
...
CREATE TABLE `passenger_safety` (
  ...
  `passenger_pin` varchar(255) DEFAULT NULL,
  `pin_verification` tinyint(1) NOT NULL DEFAULT 0,
  `created_at` datetime NOT NULL DEFAULT current_timestamp(),
  `updated_at` datetime NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
ALTER TABLE `passenger_safety`
  ...
  ADD UNIQUE KEY `unique_passenger` (`user_type`,`userID`);
